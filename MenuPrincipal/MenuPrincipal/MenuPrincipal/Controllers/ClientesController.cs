...
namespace Clientes.Controllers
{
	...
	public class ClientesController
	{
		...
		{
			string sql = "SELECT id_cliente AS 'CURP', Nombre AS 'NOMBRE', Apellido_paterno AS 'APELLIDO PATERNO',Apellido_materno AS 'APELLIDO MATERNO',Direccion AS 'DIRECCION',telefono AS 'TELEFONO',localidad AS 'LOCALIDAD' FROM Clientes WHERE Nombre LIKE'" + cliente + "%'";
		...
		{
			string sql = string.Format("UPDATE Clientes SET Nombre='{1}', Apellido_paterno='{2}', Apellido_materno='{3}', Direccion='{4}', telefono='{5}', localidad='{6}' WHERE id_cliente='{0}';",pk,Nom,ApeP,ApeM,Direc,Tele,local);
			FrameBD.SQLIDU(sql);
		}
